/***
|''Name:''|AETSearch|
|''Description:''|Adds search boxes to the AET plugin default behaviour|
|''Author:''|JonRobson|
|''Version:''|0.8.2|
|''Requires''|AETPlugin|
...
aet.extensions.search = {
	setupSearchbox: function(place, tiddlerobj, metaDataName, handler, options){
		...
		if(!selected) {
			var qsvalue = aet.getVariableFromQueryString(metaDataName);
			selected = qsvalue ? qsvalue : selected;
		}
	...
	createSearchBox: function(place, fieldName, values, initialValue, action, options) {
		...
		$("<input />").attr("type", "text").attr("name", fieldName).val(initialValue).
			autocomplete(whatyousee, options).result(handler).appendTo(place);
	}
};
aet.controlTypes.search = function(place,tiddler,metaDataName,options){
	if(!options) {
		return;
	}
	var handler = function(value) {
		aet.setMetaData(tiddler.title,metaDataName,value,options.autosavechanges);
	};
	var extension = aet.extensions.search;
