...
|''Source''|https://github.com/jdlrobson/TiddlyWiki/raw/master/plugins/AET/plugins/AETPluginDate.js|
!Usage
...
var ext = config.macros.aet.extensions.date = {
	format: "0DD/0MM/YYYY",
	_inputToYYYYMMDD: function(dmy) {
		if(dmy) {
			var d = "00", m = "00", y = "00";
			var separator;
			if(dmy.indexOf("/") > -1) {
				separator = "/";
			} else if(dmy.indexOf("-") > -1) {
				separator = "-";
			}
			if(separator) {
				dmy = dmy.split(separator);
				if(dmy.length == 3) {
					d = dmy[0] || d;
					m = dmy[1] || m;
					y = dmy[2] || y;
			...
			d = d.length == 1 ? "0" + d : d;
			m = m.length == 1 ? "0" + m : m;
			y = y.match(/[0-9]{4}$/) ? y : false;
			m = m.match(/[0-9]{2}$/) ? m : false;
			d = d.match(/[0-9]{2}$/) ? d : false;
			if(y && d && m) {
				val = y + m + d + "0000";
			} else {
				val = false;
	...
	_changeDate: function(ev, title, metaDataName, autosavechanges) {
		var dmy = $(ev.target).val();
		var val = ext._inputToYYYYMMDD(dmy);
		aet.setMetaData(title, metaDataName, val, autosavechanges);
		$("input[name=%0]".format(metaDataName)).
			val(ext._getDateString(val));
	},
	_getDateString: function(val, format) {
		format = format ||  aet.extensions.date.format;
		var noMonth = val.match(/^[0-9]{4}00/);
		var noDay = val.match(/^[0-9]{4}[0-9]{2}00/);
		if(noMonth || noDay) {
			return val.substr(0, 4);
		} else {
			var date = Date.convertFromYYYYMMDDHHMM(val);
	...
	createDatePicker: function(place,title,metaDataName,autosavechanges){
		...
		var input = $("<input />").addClass("date-pick").
			attr("name", metaDataName).appendTo(div);
		$("<div />").addClass("clearboth").appendTo(div);
		$(function() {
			var start = aet.getMetaData(title,metaDataName);
			start = start ? ext._getDateString(start) : "";
			$(input).datePicker({ startDate: '01/01/1600' }).val(start).trigger('change').
				change(function(ev) {
					ext._changeDate(ev, title, metaDataName, autosavechanges);
				});
		});
	}
};
